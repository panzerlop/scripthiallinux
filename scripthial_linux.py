...
class MouseInput:
    ...
    def click(self):
        self.__send_input(1, 0x110, 1)
        libc.usleep(50000)
        self.__send_input(1, 0x110, 0)
...
class Process:
    ...
    def get_process_maps(self, pid, name):
        ...
        a1 = self.__get_elf_address(a0, 0x02)
    ...
    def read_i16(self, address, length=0x02):
        buffer = c_int16()
        libc.pread(self.handle, pointer(buffer), length, c_long(address))
        return buffer.value
    ...
    def write_i16(self, address, value):
        ...
        return libc.pwrite(self.handle, pointer(buffer), 0x02, c_long(address))
    ...
    def read_vec3(self, address):
        buffer = Vector3()
        libc.pread(self.handle, pointer(buffer), 0x0C, c_long(address))
        return buffer
    ...
    def read_string(self, address, length=0x78):
...
class InterfaceTable:
    def __init__(self, name):
        self.table_list = mem.read_i64(
            mem.get_export(mem.get_library(name), "s_pInterfaceRegs")
        )
    ...
    def get_interface(self, name):
        ...
        while a0 != 0:
            if (
                ...
                == mem.read_string(mem.read_i64(a0 + 8))[0:-3]
            ):
                a0 = mem.read_i64(a0)
...
class NetVarTable:
    def __init__(self, name):
        ...
        while a0 != 0:
            ...
            if name.encode("ascii", "ignore") == mem.read_string(
                mem.read_i64(a1 + 0x18)
            ):
                self.table = a1
                return
...
class ConVar:
    def __init__(self, name):
        self.address = 0
        a0 = mem.read_i64(mem.read_i64(mem.read_i64(vt.cvar.table + 0x70)) + 8)
        while a0 != 0:
            if name.encode("ascii", "ignore") == mem.read_string(
                mem.read_i64(a0 + 0x18)
            ):
                self.address = a0
                return
            a0 = mem.read_i64(a0 + 8)
...
class NetVarList:
    ...
    def __get_entity_list():
        return vt.entity.table - mem.read_i32(vt.entity.function(4) + 3) + 8

    @staticmethod
    def __get_client_state():
        a0 = vt.engine.function(18)
        a1 = mem.read_i32(a0 + 0x11 + 1) + 0x16  # call 0x35da0
        a2 = mem.read_i32(a0 + a1 + 5 + 3) + 0x0C  # lea rax, [rip+0x2b21c84]
        a2 = mem.read_i64(a0 + a1 + a2 + 8)  # mov rax, QWORD PTR[rax+rdi+0x8]
        a2 += 8  # add rax, 0x8
        return a2  # ret
    ...
    def __init__(self):
        ...
        self.getLocalPlayer = mem.read_i32(vt.engine.function(0x0C) + 0x11)
        self.dwViewAngles = mem.read_i32(vt.engine.function(18) + 0x1A)

        self.dwMaxClients = mem.read_i32(vt.engine.function(20) + 0x0C)
        self.dwState = mem.read_i32(vt.engine.function(0x1A) + 0x0C)
        self.dwButton = mem.read_i32(vt.input.function(15) + 0x19)
        self.dwInput = mem.read_absolute(vt.client.function(0x10), 3, 7)
...
class Engine:
    ...
    @staticmethod
    def is_in_game():
        return mem.read_i8(nv.clientState + nv.dwState) >> 0x02
...
class Math:
    ...
    def vec_normalize(vec):
        radius = 1.0 / (
            math.sqrt(vec.x * vec.x + vec.y * vec.y + vec.z * vec.z) + 1.192092896e-07
        )
        vec.x *= radius
        vec.y *= radius
        vec.z *= radius
...
def get_crosshair_target(player):
    ...
    return (
        player.get_team_num() != cross_target.get_team_num()
        and cross_target.get_health() > 0
    )
...
if __name__ == "__main__":
    ...
    print("    Creator:            github.com/panzerlop")
